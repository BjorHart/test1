...
class TwinRuntime:
    ...
    def __init__(self, model_path, log_path=None, twin_runtime_library_path=None, log_level=LogLevel.TWIN_LOG_WARNING):
        ...
        if twin_runtime_library_path is None:
            library_path = os.path.join(local_path, twin_runtime_library)
            self._twin_runtime_library = cdll.LoadLibrary(library_path)
        else:
            self.twin_runtime_library_path = twin_runtime_library_path
            self._twin_runtime_library = cdll.LoadLibrary(twin_runtime_library_path)
    ...
    def twin_instantiate(self):
        if self.is_model_opened is False:
            raise TwinRuntimeError("The model has to be opened before instantiation!")
...
def build_empty_ctype_2d_array(num_rows, number_of_columns):
    ...
    input_data = (POINTER(c_double) * num_rows)(*[row_elements() for _ in range(num_rows)])

    return input_data


def build_ctype_2d_array(num_rows, source_df):
    input_data = (POINTER(c_double) * num_rows)()
